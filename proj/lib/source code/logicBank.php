...
class logicBank {
    ...
    public function getBankid($a_sess, $a_bankid ,&$a_out_apd, &$a_err, $a_trans = TRANS_ON) {
        // Start BLOCK A
            $l_rtn = 0;
    ...
    public function getRecId($a_sess, $a_recid ,&$a_out_apd, &$a_err, $a_trans = TRANS_ON) {
        // Start BLOCK A
            $l_rtn = 0;
		...
		$l_valCord = new validBank();
		if ( $l_valCord->check_RECID($a_recid,$a_out_apd,$a_err,array('required' => 1)))
		{
        ...
        // Start BLOCK E
            ...
            if ($l_rtn < 0) {
				if($l_rtn == ERR_DB_TOO_MUCH){
					$a_err[] = array("what" => 'Data too much', "how" =>'Data too much', "why" => "Have more than one bankid = $a_recid in DB");
				}
				elseif($l_rtn == ERR_DB_NOT_FOUND){
					$a_err[] = array("what" => 'Data not found', "how" =>'Data not found', "why" => "Don't have any data with bankid = $a_recid in DB");
				}
                $l_db_con->disconnect(DB_NG);
                return $l_rtn;
            }
	...
	public function add($a_sess, $a_in_apd, &$a_out_apd, &$a_err, $a_trans = TRANS_ON) {
		...
		// Start BLOCK D
			...
			if (count($l_ret_list) >= 1) {
				// エラー処理を作成
				$a_err[] = array("what" => "ユーザ名", "how" => $a_in_apd->getDBDBank()->getData($l_dbdBank::DBD_BANKID), "why" => "db.duplicate", "level" => 4);
				return -1;
			}

			// check row_ntn 
			if(count($l_ret_list) <= 0) {
				$l_where2 = $l_dbdBank::DBD_ROW_NTH . " = $l_row ";

				debug_log("select where = " . $l_where2);

				$l_ret_list2 = array();

				$l_rtn2 = $l_svcBank->Select($l_dbh, $l_where2, null, null, $l_ret_list2);
				debug_log("select ret = " . $l_rtn2);
				if ($l_rtn2 < 0) {
					// データが見つからないエラー以外の場合はエラー終了。
					if ($l_rtn2 !== ERR_DB_NOT_FOUND) {
						$l_db_con->disconnect(DB_NG);
						return $l_rtn2;
					}
				}
				// 同一ユーザ名が登録されている場合はエラー
				if (count($l_ret_list2) >= 1) {
					// エラー処理を作成
					$a_err[] = array("what" => "ユーザ名", "how" => $a_in_apd->getDBDBank()->getData($l_dbdBank::DBD_ROW_NTH), "why" => "db.duplicate", "level" => 4);
					return -1;
				}
			}
		...
		// Start BLOCK E
			...
			$l_dbdBank->setData($l_dbdBank::DBD_SYS_USER_ID, $a_sess['Bank_ID']);
			$l_dbdBank->setData($l_dbdBank::DBD_REG_DATE, $l_date);
			$l_dbdBank->setData($l_dbdBank::DBD_REG_USER_ID, $a_sess['Bank_ID']);
			$l_dbdBank->setData($l_dbdBank::DBD_UPD_DATE, $l_date);
			$l_dbdBank->setData($l_dbdBank::DBD_UPD_USER_ID, $a_sess['Bank_ID']);
	...
	public function del($a_sess, $a_in_apd, &$a_out_apd, &$a_err, $a_trans = TRANS_ON) {
		...
		// Start BLOCK E
			...
			$l_dbdBank->setData($l_dbdBank::DBD_UPD_DATE, $l_date);
			$l_dbdBank->setData($l_dbdBank::DBD_UPD_USER_ID, $a_sess['Bank_ID']);
			$l_dbdBank->setData($l_dbdBank::DBD_DEL_FLG, $del_flg);

	
			$l_rtn = $l_svcBank->Delete($l_dbh, $l_dbdBank);
