...
class FVDefault extends FVString
{
	// @param a_in    確認する文字列
	...
	// @param a_arr_def 既定値の配列
	// @return  -1:エラー有り、0:正常、1:値を修正して（全角、半角など）正常
	//	
	public static function valid($a_in, $a_arg1, $a_arg2, &$a_out, $a_flag, $a_required, $a_fieldName, $a_errorInfo, $a_arr_def)
	{
		...
		// 入力値と既定値をチェック
		if ( in_array($a_in, $a_arr_def) === false )
		{
			$a_errorInfo->addError($a_fieldName, $a_in, PVType::rootErrorMsg . ".data.default");
			$l_ret = -1;
			return $l_ret ;
		}

		// 文字列チェック
		$l_ret = FVString::validate($a_in, $a_arg1, $a_arg2, $a_out, $a_flag, $a_required, $a_fieldName, $a_errorInfo);
		return $l_ret;
	}
} // CLASS-EOF
?>
