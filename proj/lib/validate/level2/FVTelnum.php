...
class FVTelnum extends FVString
{
	// @param a_in    確認する文字列
	...
	public static function valid($a_in, $a_arg1, $a_arg2, &$a_out, $a_flag, $a_required, $a_fieldName, $a_errorInfo)
	{
		...
		$tel_str = true;
		// 電話番号のチェック(ハイフンなし)
		if (!preg_match("/^0\d{9,10}$/",$a_in)) {
			debug_log("ハイフンなし NG");
			$tel_num = false;
		}

		// 電話番号のチェック(ハイフンあり)
		if (!preg_match("/^[0-9]{2,4}-[0-9]{2,4}-[0-9]{3,4}$/",$a_in)) {
			debug_log("ハイフンあり NG");
			$tel_str = false;
		}
		// 電話番号チェックで両方ともエラーの場合はエラーとする。
		if ($tel_num === false && $tel_str === false) {
			$a_errorInfo->addError($a_fieldName, $a_in, PVType::rootErrorMsg . ".data.tel");
			$l_ret = -1;
			return $l_ret;
		}
