...
create table cord (
	...
	,bankid	INTEGER	NOT NULL	-- バンクコード
	,bank_no	VARCHAR(16)	NOT NULL	-- バンク内管理番号
	,receipt_bankid	INTEGER	-- 調整バンクコード
	...
	, constraint cord_pkey primary key ( recid )
	
	, constraint cord_unique1 UNIQUE(sysid)
, constraint cord_unique2 UNIQUE(bankid,bank_no,receipt_bankid)
);
